...
namespace _RagdollCharacterMechanic.Scripts
{
    ...
    public class RagdollCharacter : CharacterBase
    {
        ...
        private float _standUpTimeElapsed;
        ...
        private void HandleCollision(IHit hit)
        {
            ...
            Debug.Log($"Hit by {hit.HitBone.name} with direction {hit.HitDir} at position {hit.HitPoint}");
            OnHit?.Invoke(hit);
            TriggerRagdollState(hit.HitPoint);
        }
        ...
        private void TriggerStandUpState()
        {
            ...
            string animationState = _ragdollModule.IsFacingUp()
                ? _standUpFaceUpAnimationState
                : _standUpFaceDownAnimationState;
            
            _animationModule.PlayAnimation(animationState);

            // Get the actual animation duration
            _standUpDuration = GetStandUpAnimationDuration();
            _standUpTimeElapsed = 0f;
        ...
        {
            _standUpTimeElapsed += Time.deltaTime;
            
            if (_standUpTimeElapsed >= _standUpDuration)
            {
                if(_logsEnabled) _logger.Log("RAGDOLL_CHARACTER", $"Stand up completed after {_standUpTimeElapsed}s");
                TriggerLocomotionState();
            }
        }
        ...
        {
            Debug.Log($"Spawn Impact Effect at {position}");
        ...
        private float GetStandUpAnimationDuration()
        {
            string clipName = _ragdollModule.IsFacingUp() 
                ? _standUpFaceUpClipName 
                : _standUpFaceDownClipName;
            
            foreach (var clip in _animationModule.GetAnimationClips())
            {
                if (clip.name == clipName)
                {
                    if(_logsEnabled) _logger.Log("RAGDOLL_CHARACTER", $"Stand up animation duration: {clip.length}s");
                    return clip.length;
                }
            }
            
            // Fallback duration if clip not found
            Debug.LogWarning($"Stand up animation clip '{clipName}' not found. Using default duration.");
            return 1.5f;
        }
    }
}